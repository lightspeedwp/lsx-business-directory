...
function lsx_randomUUID() {
		var s = [], itoh = '0123456789ABCDEF';
		for (var i = 0; i <6; i++) s[i] = Math.floor(Math.random()*0x10);
		return s.join('');
}
var lsx_field_callbacks = [];
jQuery('document').ready(function($){
	// add row
	$('body').on('click', '.lsx-add-group-row', function(){
		var clicked = $( this ),
			rowid = lsx_randomUUID(),
			...
			for(var callback in lsx_field_callbacks){
				if( typeof window[lsx_field_callbacks[callback]] === 'function'){
					window[lsx_field_callbacks[callback]]();
				}
			}

	});
	$('body').on('click', '.lsx-removeRow', function(){
		$(this).next().remove();
		$(this).remove();
		////console.log(this);
	});
	// tabs
	$('body').on('click', '.lsx-metabox-config-nav li a, .lsx-shortcode-config-nav li a, .lsx-settings-config-nav li a, .lsx-widget-config-nav li a', function(){
