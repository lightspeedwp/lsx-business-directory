...
jQuery( function($){
	...
	init_magic_tags = function(){
		...
		magicfields.each(function(k,v){
			...
			if(input.hasClass('lsx-conditional-value-field')){
				wrapper.width('auto');
			}
	...
	lsx_record_change = function(){
		// hook and rebuild the fields list
		jQuery(document).trigger('record_change');
		jQuery('#lsx-id').trigger('change');
		if( config_object ){
			jQuery('#lsx-field-sync').trigger('refresh');
		}
	}
	
	lsx_canvas_init = function(){

		if( !lsx_canvas ){
			// bind changes
			jQuery('#lsx-main-canvas').on('keydown keyup change','input, select, textarea', function(e) {
				config_object = jQuery('#lsx-main-form').formJSON(); // perhaps load into memory to keep it live.
				jQuery('#lsx-live-config').val( JSON.stringify( config_object ) ).trigger('change');
			});
			// bind editor
			lsx_init_editor();
			lsx_canvas = jQuery('#lsx-live-config').val();
			config_object = JSON.parse( lsx_canvas ); // perhaps load into memory to keep it live.
		}
		...
		if( $('.lsx-group-wrapper').length ){
			$( ".lsx-group-wrapper" ).sortable({
				handle: ".sortable-item",
				update: function(){
					jQuery('#lsx-id').trigger('change');
				}
			});
			$( ".lsx-fields-list" ).sortable({
				handle: ".sortable-item",
				update: function(){
					jQuery('#lsx-id').trigger('change');
				}
	...
	lsx_get_default_setting = function(obj){
		...
		jQuery('#lsx-live-config').val( JSON.stringify( config_object ) );
		jQuery('#lsx-field-sync').trigger('refresh');
	}
	...
	// trash 
	$(document).on('click', '.lsx-card-actions .confirm a', function(e){
		e.preventDefault();
		var parent = $(this).closest('.lsx-card-content');
	...
	// bind tabs
	$(document).on('click', '.lsx-nav-tabs a', function(e){
		...
		$('.lsx-nav-tabs .current').removeClass('current');
		$('.lsx-nav-tabs .active').removeClass('active');
		$('.lsx-nav-tabs .nav-tab-active').removeClass('nav-tab-active');
		...
		$('.lsx-editor-panel').hide();
		$( tab_id ).show();
		
		if( lsx_code_editor ){
			lsx_code_editor.toTextArea();
			lsx_code_editor = false;
		}

		if( $( tab_id ).find('.lsx-code-editor').length ){

			lsx_init_editor( $( tab_id ).find('.lsx-code-editor').prop('id') );
			lsx_code_editor.refresh();
			lsx_code_editor.focus();
		}

		jQuery('#lsx-active-tab').val(tab_id).trigger('change');
	...
	$(document).on('change', '.checkboxes-list', function(e){
	...
	$(document).on('change', '.check-all', function(){
		...
		$('.checkboxes-list').prop('checked', $(this).prop('checked') );
		lsx_record_change();
	});
	...
	window.onbeforeunload = function(e) {
		...
		if( lsx_canvas && lsx_canvas !== jQuery('#lsx-live-config').val() ){
			return true;
		}
	};
...
function lsx_init_editor(el){
	if( !jQuery('#' + el).length ){
		return;
	}	
	// custom modes
	var mustache = function(lsx, state) {

		var ch;

		if (lsx.match("{{")) {
			while ((ch = lsx.next()) != null){
				if (ch == "}" && lsx.next() == "}") break;
			}
			lsx.eat("}");
			return "mustache";
		}
	...
	lsx_code_editor = CodeMirror.fromTextArea(document.getElementById(el), options);
	lsx_code_editor.on('keyup', tagFields);
	lsx_code_editor.on('blur', function(cm){
		cm.save();
		jQuery( cm.getInputField() ).trigger('change');
	});

	return lsx_code_editor;

}
(function() {
	"use strict";

	if( typeof CodeMirror === 'undefined' || lsx_canvas === false ){
		return;
	}
			...
			if(token.type){
				...
				if( token.type ){
					...
					if( in_entry ){
						...
						jQuery('.lsx-autocomplete-in-entry-' + token.type).each(function(){
						...
						jQuery('.lsx-autocomplete-out-entry-' + token.type).each(function(){
							var field = jQuery(this);
							fields[field.data('slug')] = field.data('label');
							//fields["#each " + field.data('slug')] = "#each " + field.data('label');
							//if( !in_if ){
